...
func (uq *UserQuery) QueryLogs() *OperateLogQuery {
	query := (&OperateLogClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithLogs(opts ...func(*OperateLogQuery)) *UserQuery {
	query := (&OperateLogClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLogs; query != nil {
		if err := uq.loadLogs(ctx, query, nodes,
			func(n *User) { n.Edges.Logs = []*OperateLog{} },
			func(n *User, e *OperateLog) { n.Edges.Logs = append(n.Edges.Logs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadLogs(ctx context.Context, query *OperateLogQuery, nodes []*User, init func(*User), assign func(*User, *OperateLog)) error {
	...
	query.withFKs = true
	query.Where(predicate.OperateLog(func(s *sql.Selector) {
		s.Where(sql.InValues(user.LogsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
